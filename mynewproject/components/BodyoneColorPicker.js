...
import {toggleBodyoneColorModal,
  updateBodyoneTextColor
} from '../actions/card.js'
import HsvColorPicker from 'react-native-hsv-color-picker';
class BodyoneColorPicker extends Component{
  constructor(props) {
    super(props);
    this.state = {
      hue: 0,
  ...
  onSatValPickerChange({ saturation, value }) {
    ...
    this.props.updateBodyoneTextColor({
        sat: saturation,
        val: value,
        hue: this.state.hue,
      });
  ...
  return(
    ...
    <View style={{  justifyContent: 'center', alignItems:'center', flex:1}}>
      <Modal animationType={'slide'} transparent={true} visible={this.props.card.isBodyoneColorModalVisible === true}>
          <View style={{ width: '100%', height: '100%', flex: 1, justifyContent: 'flex-end', alignItems: 'center' }}>
              ...
              <View style={{ position: 'absolute', width: '100%', height: '45%', backgroundColor: 'white', flex: 1}}>
                 ...
                 <AntDesign name="close" style = {styles.menuIcon} size ={24} onPress={() => {this.props.toggleBodyoneColorModal(false)}}   />
                 <View style= {styles.stat}>
                   <Text style={styles.modal_title}> Bodyone Text Color </Text>
                 </View>
                     ...
                     <Text style={{fontSize:30 ,color: this.props.card.bodyone_text_color == null ? 'red':
                       fromHsv({ h: this.props.card.bodyone_text_color.hue, s: this.props.card.bodyone_text_color.sat, v:this.props.card.bodyone_text_color.val })}}> Color </Text>
  ...
  const mapDispatchToProps = (dispatch) => {
    return bindActionCreators({toggleBodyoneColorModal,updateBodyoneTextColor},dispatch)
  }
  ...
  export default connect(mapStateToProps,mapDispatchToProps)(BodyoneColorPicker )
